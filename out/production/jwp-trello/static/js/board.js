var BOARD = (function (window) {
  ...
  var deckTemplate = Handlebars.compile(Template.deck),
    ...
    memberTemplate = Handlebars.compile(Template.member);
  ...
  function getBoardUsers() {
    ...
    let boardId = document.querySelector('.board')

    $.ajax({
      type: "get",
      url: "/api/users/" + document.querySelector('.board-name').id,
      beforeSend: (xhr) => {
    ...
    $.ajax({
      ...
      success: (res) => {
        if(res.length === 0) {
          ...
          var deck = deckTemplate({"id":res.id, "value": deckTitle});
  ...
  function openCardModal(e) {
    ...
    let cardId = e.target.closest(".deck-card").getAttribute("data-item");
    console.log("cardId: ", cardId);

    $("#modal").modal('open');
    var title = $(e.target).text();
    $(".card-title-in-modal").text(title);
    $(".card-title-in-modal").attr("data-item", cardId);
